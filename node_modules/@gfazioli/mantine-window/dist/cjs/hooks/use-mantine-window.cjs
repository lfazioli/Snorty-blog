...
function useMantineWindow(props) {
  ...
  const [isVisible, setIsVisible] = React.useState(opened ?? false);
  const [zIndex, setZIndex] = React.useState(200);
  const key = (id || title)?.toLocaleLowerCase().replace(/\s+/g, "-") || "window";
  const [positionStorage, setPositionStorage] = hooks.useLocalStorage({
    key: persistState && `${key}-window-position`,
    defaultValue: defaultPosition,
    getInitialValueInEffect: true
  });
  const [sizeStorage, setSizeStorage] = hooks.useLocalStorage({
    key: persistState && `${key}-window-size`,
    defaultValue: defaultSize,
    getInitialValueInEffect: true
  });
  const [positionState, setPositionState] = React.useState(defaultPosition);
  const [sizeState, setSizeState] = React.useState(defaultSize);
...
exports.useMantineWindow = useMantineWindow;
//# sourceMappingURL=use-mantine-window.cjs.map
